...
<body>
    ...
    // Encontra o indicador de status do polo
    const poloRow = encontrarElementoPolo(polo);
    const statusIndicator = poloRow?.querySelector('.status-indicator');
    ...
    fetch('/admin/api/testar-conexao-polo.php', {
        ...
        body: JSON.stringify({ 
            polo: polo,
            testar_funcoes: false, // ✅ Teste básico para ser mais rápido
    ...
    .then(data => {
        ...
        if (data.success) {
            const resultado = data.resultado_geral;
            let statusClass = 'status-offline';
            let toastType = 'error';
            let mensagem = '';
            
            // 🔧 CORREÇÃO: Mapeia status da API para classes visuais
            switch (resultado.status) {
                case 'sucesso':
                    statusClass = 'status-online';
                    toastType = 'success';
                    mensagem = `✅ ${polo}: ${resultado.status_label} (${resultado.percentual}%)`;
                    break;
                    
                case 'parcial':
                    statusClass = 'status-warning'; // ✅ Amarelo para parcial
                    toastType = 'warning';
                    mensagem = `⚠️ ${polo}: ${resultado.status_label} (${resultado.percentual}%)`;
                ...
                default:
                    ...
                    toastType = 'error';
                    mensagem = `❌ ${polo}: ${resultado.status_label}`;
                    if (resultado.problemas_criticos && resultado.problemas_criticos.length > 0) {
                        mensagem += ` - ${resultado.problemas_criticos[0]}`;
                    }
                    break;
            }
            
            // Atualiza indicador visual
            atualizarStatusPolo(polo, statusClass.replace('status-', ''));
            
            // Mostra detalhes se disponíveis
            if (data.testes) {
                mostrarDetalhesStatus(polo, data);
            }
            
            showToast(mensagem, toastType);
            
        } else {
            // Falha na API
            atualizarStatusPolo(polo, 'offline');
            showToast(`❌ Erro ao testar ${polo}: ${data.message || 'Erro desconhecido'}`, 'error');
        }
    })
    .catch(error => {
        console.error('Erro no teste de conexão:', error);
        atualizarStatusPolo(polo, 'offline');
        showToast(`❌ Erro de conexão com ${polo}: ${error.message}`, 'error');
    })
    .finally(() => {
        operacaoAtiva = false;
...
function encontrarElementoPolo(polo) {
    const poloRows = document.querySelectorAll('.polo-status');
    
    for (const row of poloRows) {
        const poloText = row.querySelector('small.text-muted');
        if (poloText && poloText.textContent.trim() === polo) {
            return row;
        }
    }
    ...
    if (confirm('Deseja sincronizar todos os polos? Esta operação pode demorar alguns minutos.')) {
        ...
        .then(data => {
            ...
            setTimeout(() => {
                esconderProgresso();
                if (data.success) {
                    showToast(`Sincronização concluída! ${data.polos_sincronizados || 0} polos atualizados.`, 'success');
                    
                    // Atualiza status de todos os polos após sincronização
                    setTimeout(() => {
                        verificarStatusPolosAutomatico();
                    }, 2000);
    ...
    if (operacaoAtiva) {
        showToast('Verificação já em andamento', 'warning');
        return;
    }
    ...
    // Usa a verificação automática mas com feedback visual
    verificarStatusPolosAutomatico();
    
    // Simula progresso
    setTimeout(() => {
        showToast('Status dos polos verificado!', 'success');
        operacaoAtiva = false;
    }, 5000);
}

// 🔧 NOVA: Função para inicialização quando a página carrega
function inicializarVerificacaoPolos() {
    console.log('🚀 Inicializando verificação de polos...');
    
    // Verifica status inicial após 2 segundos
    setTimeout(() => {
        verificarStatusPolosAutomatico();
    }, 2000);
    ...
    setInterval(() => {
        if (!operacaoAtiva) {
            console.log('🔄 Verificação periódica de polos...');
            verificarStatusPolosAutomatico();
        }
    }, 300000); // 5 minutos
}

// 🔧 NOVA: Estilos CSS adicionais para melhor visualização
...
// 🔧 NOVA: Debug e logging aprimorado
function logStatusPolo(polo, status, detalhes = {}) {
    const timestamp = new Date().toLocaleTimeString();
    console.log(`[${timestamp}] 📊 Polo ${polo}:`, {
        status: status,
        detalhes: detalhes,
        elemento_encontrado: !!encontrarElementoPolo(polo)
    });
    
    // Salva no localStorage para debug
    try {
        const logs = JSON.parse(localStorage.getItem('polo_status_logs') || '[]');
        logs.push({
            timestamp: Date.now(),
            polo: polo,
            status: status,
        ...
        localStorage.setItem('polo_status_logs', JSON.stringify(logs));
    } catch (e) {
    ...
    if (poloRow) {
        ...
        if (statusIndicator) {
            ...
            statusIndicator.classList.remove('status-online', 'status-offline', 'status-warning', 'status-loading');
            
            // Adiciona nova classe
            statusIndicator.classList.add(`status-${status}`);
            
            console.log(`Status do polo ${polo} atualizado para: ${status}`);
        }
    } else {
        console.warn(`Elemento do polo ${polo} não encontrado na página`);
    }
}

// 🔧 NOVA: Função para mostrar detalhes do status em tooltip ou modal
function mostrarDetalhesStatus(polo, dadosTeste) {
    const poloRow = encontrarElementoPolo(polo);
    
    if (poloRow) {
        // Atualiza ou cria tooltip com detalhes
        const detalhes = [];
        
        if (dadosTeste.testes.conectividade) {
            const conn = dadosTeste.testes.conectividade;
            detalhes.push(`Conectividade: ${conn.success ? '✅' : '❌'} (${conn.tempo_total || 0}ms)`);
        }
        
        if (dadosTeste.testes.autenticacao) {
            const auth = dadosTeste.testes.autenticacao;
            const statusAuth = auth.status || (auth.success ? 'ok' : 'erro');
            detalhes.push(`Token: ${getStatusIcon(statusAuth)} ${auth.message || statusAuth}`);
        }
        
        if (dadosTeste.testes.performance) {
            const perf = dadosTeste.testes.performance;
            detalhes.push(`Performance: ${perf.classificacao || 'N/A'} (${perf.tempo_medio || 0}ms)`);
        }
        
        // Adiciona título com detalhes
        poloRow.title = `Último teste: ${new Date().toLocaleString()}\n${detalhes.join('\n')}`;
        
        // Se bootstrap tooltip estiver disponível, atualiza
        if (typeof bootstrap !== 'undefined' && bootstrap.Tooltip) {
            const existingTooltip = bootstrap.Tooltip.getInstance(poloRow);
            if (existingTooltip) {
                existingTooltip.dispose();
            }
            new bootstrap.Tooltip(poloRow);
        }
    }
}

// 🔧 NOVA: Função auxiliar para ícones de status
...
// 🔧 NOVA: Função para verificar status de todos os polos automaticamente
function verificarStatusPolosAutomatico() {
    console.log('🔄 Verificação automática de status dos polos iniciada');
    
    const poloRows = document.querySelectorAll('.polo-status');
    const polos = [];
    
    // Extrai nomes dos polos da página
    poloRows.forEach(row => {
        const poloText = row.querySelector('small.text-muted');
        if (poloText) {
            polos.push(poloText.textContent.trim());
        }
    });
    
    if (polos.length === 0) {
        console.log('Nenhum polo encontrado para verificar');
        return;
    }
    
    // Verifica cada polo com delay para não sobrecarregar
    polos.forEach((polo, index) => {
        setTimeout(() => {
            verificarStatusUnicoPolo(polo);
        }, index * 1000); // 1 segundo de delay entre cada teste
    });
}

// 🔧 NOVA: Verifica status de um polo específico (versão silenciosa)
function verificarStatusUnicoPolo(polo) {
    fetch('/admin/api/testar-conexao-polo.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        ...
        body: JSON.stringify({ 
            polo: polo,
            testar_funcoes: false,
            salvar_resultado: false // Não salva verificações automáticas
        })
    })
    .then(response => response.json())
    .then(data => {
        if (data.success && data.resultado_geral) {
            const resultado = data.resultado_geral;
            let statusClass;
            ...
            atualizarStatusPolo(polo, statusClass);
            console.log(`✅ Status do polo ${polo}: ${resultado.status} (${resultado.percentual}%)`);
        } else {
            atualizarStatusPolo(polo, 'offline');
            console.log(`❌ Polo ${polo}: ${data.message || 'Erro'}`);
        }
    })
    .catch(error => {
        atualizarStatusPolo(polo, 'offline');
        console.log(`❌ Erro ao verificar ${polo}:`, error.message);
    });
}
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // NOVA INICIALIZAÇÃO DOS POLOS
            console.log('✅ Inicializando sistema de teste de polos...');
    
            // Adiciona estilos
            adicionarEstilosStatus();
    
            // Inicializa verificação
            inicializarVerificacaoPolos();

             // Adiciona event listeners para botões de teste
            document.querySelectorAll('button[onclick*="testarConexao"]').forEach(button => {
            const onclick = button.getAttribute('onclick');
            const match = onclick.match(/testarConexao\('([^']+)'\)/);
            ...
            button.title = `Testar conexão com ${polo}`;
        }
    });


    
            console.log('🎯 Sistema de teste de polos inicializado com sucesso!');
        });
